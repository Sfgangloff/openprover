...
def _build_principles(*, lean_mode: str, has_lean: bool,
    ...
    principles = (
        ...
        "- **Build on stored work.** When spawning workers for formalization, reference relevant repo items "
        "using [[slug]] syntax in the task description. Workers automatically receive the full content of "
        "any [[slug]] you reference. If previous workers stored helper lemmas, reference them so the next worker "
        "can build on that foundation instead of starting from scratch. Check the REPOSITORY index to see what's available.\n"
        ...
        "- **Don't loop on reads.** Reading items or the theorem gives you the same content each time — "
        "if you just read something, don't read it again before taking a productive action. "
        "A productive step is one that creates new information: spawning a worker, writing items, or submitting a proof. "
        "If you're unsure what to do next, update the whiteboard with your current thinking and spawn a worker "
        "to explore the next open question — don't re-read state hoping for inspiration.\n"
    )
    if not isolation:
        principles += (
...
def worker_system_prompt(*, lean_worker_tools: bool = False) -> str:
    ...
    base = (
        ...
        "If you get stuck, return what you have so far — partial progress is valuable, but only if it's concrete. "
        "Never return empty-handed. At minimum, report: "
        "(1) what specific sub-steps you completed, "
        "(2) the exact obstacle blocking further progress (e.g., a specific Lean error, a missing lemma, a gap in the proof), "
        "(3) any useful intermediate results (store them with lean_store if they compile). "
        "'I cannot complete this' with no specifics is not acceptable — identify the FIRST concrete blocker.\n"
        "\n"
        "If you have attempted the same approach multiple times without success (e.g., 3+ failed "
        "lean_verify attempts on similar code, or 3+ searches that return no useful results), "
        "stop and report back. Describe what you tried, why it failed, and what the blocker is. "
        "The planner can adjust strategy — don't keep retrying the same failing approach.\n"
        "\n"
    ...
    if lean_worker_tools:
        base += (
            ...
            "\n"
            "**Search tips:**\n"
            "- Search for **building-block concepts**, not full theorem names or problem titles. "
            "Good: `lean_search('convexHull')`, `lean_search('Finset.card')`, `lean_search('AffineIndependent')`. "
            "Bad: `lean_search('Putnam 1962 A1')`, `lean_search('five points convex quadrilateral')`.\n"
            "- If a search returns no useful results, try **synonyms or related concepts** rather than "
            "rephrasing the same query. E.g., if `convex quadrilateral` yields nothing, try `convexHull`, "
            "`extremePoints`, `Convex.strictConvex`.\n"
            "- After 3 fruitless searches on the same concept, stop searching and try to prove it from "
            "more basic lemmas you already found.\n"
            "\n"
            ...
            "Never pass code containing `sorry` to `lean_store` — only fully proven code. "
            ...
            "\n"
            "**Know when to stop.** If you've made several attempts at a lemma and keep hitting "
            "the same errors, report what you have. Include: (1) what compiles so far (lean_store it), "
            "(2) what specific Lean error you're stuck on, (3) what Mathlib lemmas or tactics you think "
            "are needed but couldn't find. This is more valuable than 20 more failed attempts.\n"
            "\n"
